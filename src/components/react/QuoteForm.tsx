...
function FloatingDateField({ label, error, isTouched, value, register }: { label: string; error?: string; isTouched: boolean; value: string; register: UseFormRegister<QuoteFormValues> }) {
  ...
  return <div><label className="relative block"><input id="moveDate" type="date" min={todayMoveDate()} {...register("moveDate")} aria-invalid={Boolean(error)} aria-describedby={(error || isValid) ? messageId : undefined} className={`h-14 w-full rounded-lg border bg-white px-4 pb-1 pt-5 font-sans text-sm text-text-strong outline-none transition-colors focus:ring-2 ${error ? "border-alert focus:border-alert focus:ring-alert/15" : isValid ? "border-success focus:border-success focus:ring-success/15" : "border-ink/15 focus:border-brand-primary focus:ring-brand-primary/15"}`} /><span className="pointer-events-none absolute left-4 top-2 bg-white px-1 font-sans text-xs text-text-subtle">{label}</span>{isValid && <span className="absolute right-10 top-1/2 -translate-y-1/2 text-success" aria-hidden="true">✓</span>}</label><FieldFeedback id={messageId} error={error} isValid={isValid} /></div>;
}
...
function FloatingSelectField({ label, error, isTouched, value, register }: { label: string; error?: string; isTouched: boolean; value: string; register: UseFormRegister<QuoteFormValues> }) {
  ...
  return <div><label className="relative block"><select id="homeSize" defaultValue="" {...register("homeSize")} aria-invalid={Boolean(error)} aria-describedby={(error || isValid) ? messageId : undefined} className={`h-14 w-full appearance-none rounded-lg border bg-white px-4 pb-1 pt-5 font-sans text-sm text-text-strong outline-none transition-colors focus:ring-2 ${error ? "border-alert focus:border-alert focus:ring-alert/15" : isValid ? "border-success focus:border-success focus:ring-success/15" : "border-ink/15 focus:border-brand-primary focus:ring-brand-primary/15"}`}><option value="" disabled>Select your move size</option>{MOVE_SIZES.map(([optionValue, text]) => <option key={optionValue} value={optionValue}>{text}</option>)}</select><span className="pointer-events-none absolute left-4 top-2 bg-white px-1 font-sans text-xs text-text-subtle">{label}</span><span className="pointer-events-none absolute right-4 top-1/2 -translate-y-1/2 text-text-subtle" aria-hidden="true">⌄</span>{isValid && <span className="absolute right-10 top-1/2 -translate-y-1/2 text-success" aria-hidden="true">✓</span>}</label><FieldFeedback id={messageId} error={error} isValid={isValid} /></div>;
}
